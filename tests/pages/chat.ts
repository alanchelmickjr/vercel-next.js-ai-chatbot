...
export class ChatPage {
  ...
  public async chooseModelFromSelector(chatModelId: string) {
    // Use the model-management-client to get the model by ID
    const { getModelById } = await import('@/lib/db/model-management-client');
    const model = await getModelById(chatModelId);
    ...
    expect(await this.getSelectedModel()).toBe(model.displayName);
  }
